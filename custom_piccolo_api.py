...
from piccolo_api.crud.exceptions import MalformedQuery, db_exception_handler
...
class PiccoloCRUD(Router):
    ...
    def _parse_params(self, params: QueryParams) -> t.Dict[str, t.Any]:
        ...
        array_columns = [
            ...
            if isinstance(i.value_type, list)
        ]
    ...
    async def patch_single(
        ...
        try:
            # use ORM object queries (not query builder) to make overide
            # methods work also in Piccolo Admin
            result = await cls.objects().get(cls._meta.primary_key == row_id)
