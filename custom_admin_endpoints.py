...
from piccolo_admin.translations.models import (
    Translation,
    TranslationListItem,
    TranslationListResponse,
)
from piccolo_admin.version import __VERSION__ as PICCOLO_ADMIN_VERSION
...
class TableConfig:
    ...
    def __post_init__(self):
        if self.visible_columns and self.exclude_visible_columns:
            raise ValueError(
                "Only specify `visible_columns` or " "`exclude_visible_columns`."
            )
        ...
        self.media_columns = (
            {i.column: i for i in self.media_storage} if self.media_storage else None
        )
...
class AdminRouter(FastAPI):
        ...
        *tables: t.Union[t.Type[Table], TableConfig],
        ...
        media_storage = [
            ...
            for i in itertools.chain(
                *[table_config.media_storage or [] for table_config in table_configs]
            )
        ]
    ...
    def _get_media_storage(self, table_name: str, column_name: str) -> MediaStorage:
        """
        ...
        try:
            column = table_config.table_class._meta.get_column_by_name(column_name)
        except ValueError:
            raise HTTPException(
                status_code=404,
    ...
    async def store_file(
        ...
        """
        if self.read_only:
            raise HTTPException(status_code=405, detail="Running in read-only mode.")
    ...
    async def generate_file_url(
        ...
        file_url = await media_storage.generate_file_url(
            ...
            root_url=(f"./api/media-files/{model.table_name}" f"/{model.column_name}/"),
            user=request.user.user,
        )
    ...
    async def post_single_form(self, request: Request, form_slug: str) -> t.Any:
        """
        ...
        try:
            ...
            if inspect.iscoroutinefunction(endpoint):
                response = await endpoint(request, model_instance)  # type: ignore
            else:
                response = endpoint(request, model_instance)
        except ValueError as exception:
            return JSONResponse({"custom_form_error": str(exception)}, status_code=422)

        message = response if isinstance(response, str) else "Successfully submitted"
    ...
    def get_table_list_grouped(self) -> GroupedTableNamesResponseModel:
        ...
        group_names = sorted({i.menu_group for i in self.table_configs if i.menu_group})
        response.grouped = {i: [] for i in group_names}

        for table_config in self.table_configs:
            menu_group = table_config.menu_group
            if menu_group is None:
                response.ungrouped.append(table_config.table_class._meta.tablename)
            else:
...
def create_admin(
    ...
    if auto_include_related:
        ...
        all_table_classes_with_configs: t.List[t.Union[t.Type[Table], TableConfig]] = []
